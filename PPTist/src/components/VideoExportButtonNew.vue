...
<template>
  ...
  <!-- 工作流进度模态框 - 移到根级别 -->
  <div v-if="showWorkflowProgress" class="workflow-progress-overlay">
    <div class="workflow-progress-modal" @click.stop>
      <div class="modal-header">
        <h2>视频生成进度</h2>
        <button @click="closeWorkflowProgress" class="close-btn" :disabled="workflowStatus?.status === 'running'">
          ×
        </button>
      </div>
      <div class="modal-content">
        <div class="workflow-status">
          <div v-if="workflowStatus" class="status-info">
            <h3>{{ workflowStatus.project_name }}</h3>
            <div class="current-step">
              当前步骤: {{ workflowStatus.current_step }}/{{ workflowStatus.total_steps }}
            </div>
              ...
              >
                ...
                <div class="step-info">
                  ...
                  <div class="step-progress">
                    ...
                    <span class="progress-percent">{{ Math.round(step.progress * 100) }}%</span>
                  </div>
            ...
            <div v-if="workflowStatus.status === 'completed'" class="completion-actions">
              <div class="success-message">
                <span>🎉</span>
                <span>视频生成完成！</span>
              </div>
              <button 
                v-if="workflowStatus.final_video" 
                @click="downloadVideo(workflowStatus.final_video)"
                class="download-btn"
              >
                <span>📥</span>
                <span>下载视频</span>
            ...
            <div v-if="workflowStatus.status === 'failed'" class="error-message">
              <span>💥</span>
              <span>生成失败: {{ workflowStatus.error }}</span>
            </div>
          </div>
      ...
      <div class="modal-footer">
        <button 
          @click="closeWorkflowProgress" 
          class="close-workflow-btn"
          :disabled="workflowStatus?.status === 'running'"
        >
          {{ workflowStatus?.status === 'running' ? '运行中...' : '关闭' }}
        </button>
      </div>
    </div>
  </div>
...
const startVideoExport = async () => {
  ...
  try {
    ...
    try {
      ...
      console.log('🚀 开始导出，项目名称:', projectName)
      result = await exportToBackendWithExistingThumbnails(projectName)
      // eslint-disable-next-line no-console
      console.log('✅ 导出函数完成，返回值:', result)
    }
    catch (error) {
      // eslint-disable-next-line no-console
    ...
    if (result) {
      // eslint-disable-next-line no-console
      console.log('✅ 导出成功，工作流ID:', result)
      
      // 直接使用返回的工作流ID
      currentWorkflowId.value = result
...
const startStatusCheck = () => {
  ...
  statusCheckInterval.value = setInterval(async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/workflow/status/${currentWorkflowId.value}`)
      const result = await response.json()
      
      if (result.success) {
        const previousStatus = workflowStatus.value?.status
        workflowStatus.value = result.workflow
        
        // 如果状态从非完成变为完成，自动滚动到完成区域
        if (previousStatus !== 'completed' && result.workflow.status === 'completed') {
          setTimeout(() => {
            scrollToCompletionArea()
          }, 500) // 等待DOM更新后滚动
        }
...
// 滚动到完成区域
const scrollToCompletionArea = () => {
  const modalContent = document.querySelector('.workflow-progress-modal .modal-content')
  const completionActions = document.querySelector('.completion-actions')
  
  if (modalContent && completionActions) {
    // 滚动到完成区域，确保下载按钮可见
    completionActions.scrollIntoView({
      behavior: 'smooth',
      block: 'end'
    })
    ...
    setTimeout(() => {
      if (completionActions) {
        const htmlEl = completionActions as HTMLElement
...
const downloadVideo = (filename: string) => {
  try {
    const projectName = workflowStatus.value?.project_name
    if (!projectName) return
    
    const downloadUrl = `${API_BASE_URL}/api/download/${projectName}/${filename}`
