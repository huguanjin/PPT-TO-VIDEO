...
class IntegratedTTSManager:
    ...
    async def _edge_tts_synthesize(self, text: str, output_path: Path) -> Dict[str, Any]:
        ...
        if CUSTOM_EDGE_TTS_AVAILABLE:
            try:
                ...
                if output_path.exists() and output_path.stat().st_size > 1000:
                    ...
                    self.logger.info(f"自定义Edge TTS成功，文件大小: {output_path.stat().st_size} bytes")
                    return {
                        "success": True,
        ...
        # 如果自定义实现失败，回退到原始库方法（仅作为备用）
        self.logger.info("回退到原始Edge TTS库方法")
        if EDGE_TTS_AVAILABLE:
            try:
                return await self._edge_tts_library_method(text, output_path)
            except Exception as e:
                self.logger.error(f"原始Edge TTS库方法也失败: {str(e)}")
                raise Exception(f"Edge TTS完全失败: 自定义方法和库方法都不可用 - {str(e)}")
        else:
            raise ImportError("Edge TTS 不可用：自定义实现失败且官方库未安装")
        ...
        if CUSTOM_FISH_TTS_AVAILABLE:
            try:
                ...
                await loop.run_in_executor(
                    ...
                    text, 
                    str(output_path),
                    self.config.fish_character_name  # 传递角色参数
                )
...
def load_tts_config_from_app_config() -> TTSConfig:
    ...
    if app_config_path.exists():
        try:
            ...
            tts_config_data = app_config.get("tts", {})
            if tts_config_data:
                # 从app_config.json中的tts部分创建TTSConfig
                return TTSConfig(
                    preferred_engine=tts_config_data.get("preferred_engine", "edge_tts"),
                    edge_voice=tts_config_data.get("edge_voice", "zh-CN-XiaoxiaoNeural"),
                    edge_rate=tts_config_data.get("edge_rate", "medium"),
                    edge_pitch=tts_config_data.get("edge_pitch", "medium"),
                    fish_api_key=tts_config_data.get("fish_api_key", ""),
                    fish_character_name=tts_config_data.get("fish_character_name", "雷军"),
                    fish_character_id=tts_config_data.get("fish_character_id", ""),
                    openai_api_key=tts_config_data.get("openai_api_key", ""),
                    openai_voice=tts_config_data.get("openai_voice", "alloy"),
                    openai_model=tts_config_data.get("openai_model", "tts-1"),
                    azure_api_key=tts_config_data.get("azure_api_key", ""),
                    azure_region=tts_config_data.get("azure_region", ""),
                    azure_voice=tts_config_data.get("azure_voice", "zh-CN-XiaoxiaoNeural"),
                    sample_rate=tts_config_data.get("sample_rate", 22050),
                    max_retries=tts_config_data.get("max_retries", 3),
                    timeout=tts_config_data.get("timeout", 30)
                )
        except Exception as e:
            print(f"加载app_config.json中的TTS配置失败: {e}")
...
def load_tts_config_from_file(config_file_path: str = "config_data/tts_config.json") -> TTSConfig:
    ...
    if config_path.exists():
        ...
        return TTSConfig(
            preferred_engine=config_data.get("preferred_engine", "edge_tts"),
