...
class FFmpegFinalMerger:
    ...
    def _prepare_merge_parameters(self, 
        ...
        if include_subtitles and subtitle_data:
            ...
            subtitle_filename = (
                ...
                subtitle_data.get("combined_subtitle_file") or
    ...
    def _add_subtitles(self, video_path: str, subtitle_path: str, output_path: str, subtitle_config: Optional[Dict[str, Any]] = None) -> bool:
        """
    ...
    def _css_color_to_ffmpeg(self, css_color: str) -> str:
        """
        将CSS颜色格式转换为FFmpeg字幕颜色格式
        CSS: #RRGGBB -> FFmpeg: &HBBGGRR (注意BGR顺序)
        """
        try:
            # 移除 # 前缀
            ...
            # 确保是6位十六进制
            if len(css_color) == 3:
                # 将简写形式扩展为完整形式 (#RGB -> #RRGGBB)
                css_color = ''.join([c*2 for c in css_color])
            
            if len(css_color) != 6:
                self.logger.warning(f"无效的CSS颜色格式: #{css_color}，使用默认白色")
                return "&HFFFFFF"
            ...
            # 转换为BGR格式的FFmpeg颜色 (注意顺序)
            ffmpeg_color = f"&H{b:02X}{g:02X}{r:02X}"
            
            self.logger.debug(f"颜色转换: CSS #{css_color} -> FFmpeg {ffmpeg_color}")
            return ffmpeg_color
